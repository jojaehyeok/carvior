      ...
      .then((items: any[]) => {
        ...
        if (found) {
          setItem({
            ...
            auctionStartAt: found.auctionStartAt,
            auctionEndAt: found.auctionEndAt,
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6 pt-4 pb-2">
        <nav className="flex items-center gap-1.5 text-xs text-gray-400">
          ...
          <span>/</span>
          <Link href="/auction/market" className="hover:text-black transition-colors">스마트옥션</Link>
          <span>/</span>
          <span className="text-gray-600 font-semibold truncate max-w-[200px]">{item.titleEn ?? item.carNumber}</span>
        </nav>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-6">
          ...
          <div>
            ...
            <div className="flex items-center justify-end gap-4 mb-4 text-sm text-gray-400">
              ...
              <button
                onClick={() => navigator.share?.({ title: item.titleEn, url: window.location.href }).catch(() => {})}
            ...
            {item.titleKo !== item.titleEn && (
              <p className="text-sm text-gray-400 mb-2">{item.titleKo}{item.trim ? ` · ${item.trim}` : ''}</p>
            )}
            ...
            <div className="space-y-2 mb-3">
              <button
                ...
                className="w-full bg-black hover:bg-gray-800 disabled:bg-gray-100 disabled:text-gray-400 text-white font-black py-3.5 rounded-xl text-sm transition-colors flex items-center justify-center gap-2"
              >
                🔨 {closed ? (item.status === 'sold' ? '낙찰완료' : '경매 마감') : myBids.length > 0 ? '입찰 수정' : '입찰하기'}
                {myBids.length > 0 && !closed && (
                  <span className="bg-white/20 text-white text-xs font-black px-2 py-0.5 rounded-full">{myBids.length}</span>
                )}
              </button>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <div className="bg-white rounded-xl p-4">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-1.5">
                    ...
                    <p className="text-sm font-black text-gray-900">{item.accident ? '사고이력 있음' : '완전무사고'}</p>
                  </div>
                </div>
              ...
              <div className="bg-white rounded-xl p-4">
                <div className="flex items-center justify-between mb-3">
                  ...
                  {item.carHash && (
                    <a href={`/report/${item.carHash}`} target="_blank" rel="noopener noreferrer"
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-end md:items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-md p-6">
            ...
            <p className="text-sm text-gray-500 bg-gray-50 rounded-xl p-3 mb-4 font-medium">
              ...
              {item.priceKRW > 0 ? ` · ${fmtKRW(item.priceKRW)}` : ''}
            </p>
...
function DetailTabs({ item }: { item: AuctionItem }) {
  ...
  const baseSpecRows = (item.specs && item.specs.length > 0) ? item.specs : [
    ...
    { label: '배기량',    value: item.displacement || '-' },
    { label: '승차인원',  value: '5' },
    { label: '구동방식',  value: '-' },
    { label: '최초등록일', value: '-' },
        ...
        <div className="max-w-3xl">
          {inspected ? (
            <>
              ...
              {[
                ...
                <div key={section} className="mb-6">
                  ...
                  <div className="border border-gray-100 rounded-xl overflow-hidden">
                    {items.map((it, idx) => (
                      <div key={it.label} className={`flex items-center justify-between px-4 py-3 text-sm bg-white ${idx < items.length - 1 ? 'border-b border-gray-50' : ''}`}>
                        <span className="text-gray-500">{it.label}</span>
                        <span className={`font-black flex items-center gap-1 ${it.bad ? 'text-red-500' : 'text-green-600'}`}>
                          {!it.bad ? (
              ...
              {/* 타이어 게이지 */}
              {item.conditionData?.tireTread && (
                <div className="mt-2 bg-gray-50 rounded-xl p-4">
                  <p className="text-xs font-black text-gray-400 mb-3">타이어 트레드</p>
                  <div className="space-y-2">
                    <TireTread label="앞" value={item.conditionData.tireTread.front} />
                    <TireTread label="뒤" value={item.conditionData.tireTread.back} />
                  </div>
                </div>
              )}
            </>
